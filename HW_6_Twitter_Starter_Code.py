...
import secret_data as secrets # file that contains your OAuth credentials
...
def make_request(baseurl, params):
    ...
    #TODO Implement function
    BASE_URL=baseurl
    resp = requests.get(BASE_URL, params=params, auth=oauth)
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    uniquekey = construct_unique_key(baseurl, {'q': hashtag.lower(),"count":count})
    try:
        m=1
        for i in cache_dict[uniquekey]["statuses"]:
            for j in i["entities"]["hashtags"]:
               if j["text"].lower()==hashtag[1:].lower():
                   m=0
                   break
        if m==0:
                print("fetching cached data")
                res = cache_dict[uniquekey]
        # else:
        #        print("making new request")
        #        response = make_request(baseurl,params = {'q': hashtag,"count":100})
        #        cache_dict[uniquekey] = {}
        #        cache_dict[uniquekey]["statuses"] = list()
        #        if count <= len(response["statuses"]):
        #            for x in range(count):
        #                cache_dict[uniquekey]["statuses"].append(response["statuses"][x])
        #        else:
        #            cache_dict[uniquekey]["statuses"] = response["statuses"]
        #        res = cache_dict
        ...
        print("making new request")
        response = make_request(baseurl, params={'q': hashtag.lower(),"count":count})
        cache_dict[uniquekey] = {}
        cache_dict[uniquekey]["statuses"] = list()
        # if count<=len(response["statuses"]):
        #     for x in range(count):
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for i in tweet_data["statuses"]:
        for j in i["entities"]["hashtags"]:
            if j["text"].lower()==hashtag_to_ignore[1:].lower():
                continue
            else:
                count.append(j["text"])

    resp=max(count, key=count .count)
    return "#"+resp
